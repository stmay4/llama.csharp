...
{
    public class LlamaExecutor : LlamaExecutorBase
    {
        ...
        //
        //   logger:
        public LlamaExecutor(LLamaContext context)//, ILogger? logger = null)
            : base(context)//, logger)
        {
        }
            ...
            while (_is_prompt_run)
            {
                ...
                if (_embeds.Count > 0)
                {
                    ...
                    if (_pastTokensCount + _embeds.Count > Context.ContextSize)
                    {
                        ...
                        tokensKeep = tokensKeep >= 0 && tokensKeep <= _embed_inps.Count ? tokensKeep + Convert.ToInt32(Context.Vocab.ShouldAddBOS) : _embed_inps.Count;
                                                                                                                                                    ...
                                                                                                                                                    // для улучшения времени первого токена
        {
            ...
            else if (_embed_inps.Count <= _consumedTokensCount && !args.StopGeneration)
            {
        ...
        public LLamaToken? SampleOneTokenWithoutDecode(IInferenceParams inferenceParams) // бесполезный метод, а может и нет. Дает новый токен за 2 мс, если запрос уже обработан. Тогда как InferenceInternal сразу его и обрабатывает, что ухудшает время преовго токена
        {
            ...
            if (_embed_inps.Count <= _consumedTokensCount && _lastBatch.TokenCount > 0)
            {
